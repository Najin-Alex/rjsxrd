...
> **Примечание:** по умолчанию генератор создаёт только проверенные конфиги в `bypass/`. Остальные наборы (`default/`, `bypass-unsecure/`, `split-by-protocols/`, `tg-proxy/`, raw-файлы) включаются feature-флагами `--enable-*` (см. [Режимы запуска](#режимы-запуска)). Все ссылки ниже актуальны — файлы публикуются на GitHub.
...
source/               - исходный код генератора
 ...
 ├─ tests/            - unit-тесты (658 тестов, 26 файлов)
 │   ├─ __init__.py
...
Проект включает набор unit-тестов для проверки корректности работы основных модулей (658 тестов, 26 файлов):
...
Подробная документация по тестам — в [`docs/development/testing.md`](docs/development/testing.md).
