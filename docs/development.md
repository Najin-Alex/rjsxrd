...
- Параллельная загрузка из URLS, EXTRA_URLS_FOR_BYPASS, URLS_BASE64, URLS_YAML
...
- Улучшена архитектура: чистое разделение ответственности между модулями
- Добавлена поддержка ежедневно обновляемого репозитория VPN-конфигов: автоматический поиск конфигов по дате с учетом часовых поясов и поддержка base64-декодирования
- **НОВОЕ**: Улучшенная проверка безопасности с поддержкой протокол-специфичных уязвимостей (VMess legacy mode, weak ciphers и т.д.)
- **НОВОЕ**: Поддержка параллельных загрузок с ограничением числа потоков
- **НОВОЕ**: Потокобезопасное логирование с сортировкой сообщений по файлам
- **НОВОЕ**: Улучшенная дедупликация конфигов на основе host:port комбинации
